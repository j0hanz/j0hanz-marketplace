...
The constructor's `listChanged` option opens the stream and re-fetches automatically; a manual `setNotificationHandler` overrides it. Pre-`2026-07-28` servers use `subscribeResource`/`unsubscribeResource`; an era mismatch throws `METHOD_NOT_SUPPORTED_BY_PROTOCOL_VERSION`.
