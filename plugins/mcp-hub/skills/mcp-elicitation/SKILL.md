...
4. **Guard Cancellation**: Pass `ctx.mcpReq.signal` to database or HTTP calls, and verify `signal.aborted` within recursive functions or loop conditions (the transport aborts in-flight handlers on close in v2).
