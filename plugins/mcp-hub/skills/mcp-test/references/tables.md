...
| `SdkHttpError`                        | server/client | HTTP-level failure (2025-era); HTTP status on `.status`/`.statusText`, **not** `.code` (`.code` is a `SdkErrorCode` string)                                               |
...
| `InsufficientScopeError` (transport)  | client        | 403 `insufficient_scope`; extends `OAuthClientFlowError`, not `OAuthError` (SEP-2350) — distinct from `OAuthError(InsufficientScope)`                                     |

## Error Channels

See [../../mcp-server/SKILL.md](../../mcp-server/SKILL.md) "Handle Errors" for the tool vs protocol error channel distinction.
...
| `TS2589: Type instantiation is excessively deep`                                        | Multiple Zod versions in the tree. Dedupe to a single `zod ^4.2.0` (zod 3 fails quietly at runtime; 4.0–4.1 degrades silently, dropping `.describe()` — see [mcp-server](../../mcp-server/SKILL.md) Standard Schema note).                                                             |
