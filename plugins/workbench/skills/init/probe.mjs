...
// Every parse() therefore runs above the Commands section, which is the first to
// report this list: a call placed below it is a bad file nobody hears about.
// `read` also closes over `at` (defined after the walk below), so no `read` or
// `parse` call may precede `const at` — the first is `parse('package.json')`.
const unparsed = [];
...
const scoped = process.argv[2] !== undefined;
...
for (const [dir, n] of [...trees].sort((a, b) => b[1] - a[1]).slice(0, TREES_SHOWN)) {
  if (n < 2) continue;
